...
/**
  Send FF-A console log command.

  @param [in]   Buffer           Buffer to send to the console
  ...
  )
{
  ...
  if ((Buffer == NULL) || (BufferSize == 0) || (WrittenSize == NULL)) {
    return EFI_INVALID_PARAMETER;
  }

  FfaArgs.Arg0 = ARM_FID_FFA_CONSOLE_LOG_AARCH64;
  FfaArgs.Arg1 = BufferSize;

  CopyMem (&FfaArgs.Arg2, Buffer, BufferSize);

  ArmCallFfa (&FfaArgs);

  Status = FfaArgsToEfiStatus (&FfaArgs);
  if (Status == EFI_TIMEOUT) {
    *WrittenSize = FfaArgs.Arg3;
